...
teamCard = {
  ...
  computed: {
    ...
    opponent() {
      if (!this.team.h2h_info || !this.getTable.entries) {
        return null;
      }
      return this.getTable.entries.find(team => team.team_code == this.team.h2h_info.current_opponent);
    },
    benchPlayers() {
      return this.team.players.filter(player => !player.on_field);
    }
  },
  methods: {
    togglePlayer(player) {
      this.selectedPlayerId = this.selectedPlayerId === player.id ? null : player.id;
    },
    toggleBench() {
      this.benchExpanded = !this.benchExpanded;
      if (!this.benchExpanded && this.benchPlayers.some(player => player.id === this.selectedPlayerId)) {
        this.selectedPlayerId = null;
      }
    },
    pointSourceRows(player) {
      return player.point_sources
        .filter(source => source.points_total !== 0)
        .sort(this.pointSourceOrder);
    },
    getShirtUrl(player) {
  ...
  template: `
    <div class="team-card" >
      <div>
        ...
        <div class="row-container team-gameweek-row">
          ...
          <button
            v-if="benchPlayers.length"
            type="button"
            class="bench-disclosure"
            :aria-expanded="benchExpanded"
            @click="toggleBench"
          >
            <span>Bench</span>
            <svg aria-hidden="true" viewBox="0 0 12 12" fill="none" focusable="false">
        ...
        >
          <div
            v-if="player.on_field"
            class="lineup-player"
            :class="{ 'lineup-player-expanded': selectedPlayerId === player.id }"
            role="button"
            tabindex="0"
            :aria-expanded="selectedPlayerId === player.id"
            @click="togglePlayer(player)"
            @keydown.enter.prevent="togglePlayer(player)"
            @keydown.space.prevent="togglePlayer(player)"
          >
            ...
            <div class="row-container">
              <div v-if="player.play_status.type == 'playing'" class="row-container">
                ...
                <span class="dot" :class="getPointsStatusColorClass(player)">{{ getPlayerPointsString(player) }}</span>
                <player-source-icons :player="player"></player-source-icons>
              ...
              <div v-else class="row-container">
                <div v-if="player.play_status.type == 'subbed_off'" class="row-container" style="margin: auto 0px;" >
                  ...
                  <div class="row-container">
                    ...
                    <span class="dot" :class="getPointsStatusColorClass(getSubbedWith(player))">{{ getPlayerPointsString(getSubbedWith(player)) }}</span>
                <player-source-icons :player="getSubbedWith(player)"></player-source-icons>
                  </div>
                </div>
              </div>
            ...
            <div v-if="selectedPlayerId === player.id" class="inline-player-details" @click.stop>
              <div class="inline-player-heading">
                <div>
              ...
              <div
                v-if="player.point_sources.some(source => source.points_total !== 0)"
                class="player-point-breakdown"
              >
                <div
                  v-for="(source, sourceIndex) in pointSourceRows(player)"
                  :key="source.fixture + '-' + source.stat + '-' + sourceIndex"
                  class="player-point-source"
                >
                  <span class="point-source-name">{{ source.name }}</span>
                  <point-source-icon :source="source" class="point-source-detail-icon"></point-source-icon>
                  <strong class="point-source-points">
                    {{ source.points_total > 0 ? '+' : '' }}{{ source.points_total }} pts
                  </strong>
                </div>
              </div>
              <p v-else class="player-details-empty">No point breakdown available yet.</p>
            </div>
          </div>
        </div>
        ...
        <div v-if="benchExpanded && benchPlayers.length" class="bench-list">
            <div v-for="player in benchPlayers" :key="'bench-' + player.id">
              <button
                type="button"
                class="bench-player-row"
                :class="{ 'bench-player-row-expanded': selectedPlayerId === player.id }"
                :aria-expanded="selectedPlayerId === player.id"
                @click="togglePlayer(player)"
              >
                <img :src="getShirtUrl(player)" :alt="player.team.short_name + ' shirt'" />
                <span class="bench-player-name">{{ player.display_name }}</span>
                <span class="dot" :class="getPointsStatusColorClass(player)">{{ getPlayerPointsString(player) }}</span>
                <player-source-icons :player="player"></player-source-icons>
              </button>

              <div v-if="selectedPlayerId === player.id" class="inline-player-details bench-player-details">
                <div class="inline-player-heading">
                  <div>
                    <strong>{{ player.full_name }}</strong>
                    <span>{{ player.team.name }} · {{ player.team_pos }}</span>
...
app.component('point-source-icon', {
  ...
  template: `<span v-if="!['minutes', 'goals_conceded', 'saves'].includes(source.stat)" class="point-source-icon" role="img" :aria-label="source.name || source.stat" :title="source.name || source.stat">
                  ...
                  <div v-if="source.stat == 'red_cards'" v-for="x in source.amount" style="margin: auto; height: 1.25rem;">
                  ...
                  <div v-if="source.stat == 'bonus'" style="margin: auto; height: 1.25rem;">
...
app.component('player-source-icons', {
  props: { player: Object },
  computed: {
    sources() {
      return this.player.point_sources.filter(source => source.points_total !== 0)
        .sort(teamCard.methods.pointSourceOrder);
    }
  },
  template: `<span class="player-source-icons">
    <point-source-icon v-for="(source, index) in sources"
      :key="source.fixture + '-' + source.stat + '-' + index"
      :source="source"></point-source-icon>
  </span>`
});
